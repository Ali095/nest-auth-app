...
export class DbRepo {
  ...
  private findByObjectTemplate(allUsers: User[], objectTemplate: any) {
    return allUsers.filter((u: User) => Object.keys(objectTemplate).every(
      (propertyName) => u[propertyName as keyof User] == objectTemplate[propertyName],
    ));
  }

  private updateMatchingObjectProperties(obj1: any, obj2: any): any {
    for (const i in Object.keys(obj2)) {
      for (const j in Object.keys(obj1)) {
